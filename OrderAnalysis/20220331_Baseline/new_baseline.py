...
def order_selection(df_side, method,demean, order_amt_all, order_side, top, date):
    ...
    ## decide alpha bar based on previous minute
    df_temp = df_side.loc[df_side['order_amt_cumsum']<=df_side['selection_amt']]
    df_temp = df_temp.groupby('interval').agg({'alpha': 'min'}).reset_index()
    df_temp['interval_prev'] = df_temp['interval'].shift(-1)
    df_temp = df_temp[['interval_prev', 'alpha']].rename(columns = {'interval_prev': 'interval', 'alpha': 'alpha_bar'})
    df_side = df_side.merge(df_temp, on = 'interval', how = 'left')
    df_side = df_side.loc[df_side['interval'] >= 93500000000]


    df_side_top = df_side.loc[df_side['alpha']>=df_side['alpha_bar']]
    df_side_top.to_parquet('/data/home/jianw/OrderAnalysis/20220331_Baseline/%s.topOrders.%s.parquet'%(univ, date))
    return(df_side_top)
...
def saveSummary(univ, sdate, edate, inSample, top, order_cap, order_side, method, ret, wt_mta30m, wt_sta, wt_priceReal):
    ...
    #df.to_csv('/data/home/jianw/OrderAnalysis/20220331_Baseline/summary_mid/%s/new_top%s.%s.%s.orderside%s.%s-%s-%s.%s.%s.%s.csv'%(univ, top,method,ret, order_side, wt_mta30m, wt_sta, wt_priceReal, sdate, edate, inSample_dic[inSample] ), index = False)
...
if __name__ == '__main__':
    ...
    wts_priceReal = [0, 0.1, 0.2, 0.3, 0.4]
    wts_priceReal = [0.1]
    for univ in Univs:
