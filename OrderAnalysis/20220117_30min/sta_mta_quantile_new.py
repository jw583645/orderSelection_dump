...
def stamta(tuple):
    ...
    df.eval('(safeBid1p * ask1q + safeAsk1p * bid1q) / (bid1q + ask1q)')
    ...
    df_side = df_side.sort_values(by = ['interval','alp_30m', 'clockAtArrival'], ascending=[True, True, True]) 
    '''
    if order_side == 1:
...
def summary_mta(df_side):
    ...
    for i in range(10):
        ...
        df_local = df_local.loc[~df_local['sta'].isna()]
        df_local = df_local.loc[~df_local['alp_30m'].isna()]
        df_local.to_parquet('/data/home/jianw/OrderAnalysis/20220117_30min/decile_data/orderside%s/decile%s.parquet'%(order_side, i+1))
    ...
    df = pd.DataFrame({'mta_q':mta_q, 'tradeAmt': tradeAmt, 'MTA_mean': MTA_mean, 'STA_mean': STA_mean, 'ret_30m': ret_30m, 'r2': R2, 'MTA_coef': MTA_coef, 'STA_coef': STA_coef, 'Spread_coef': Spread_coef})
    #df = pd.DataFrame({'mta_q':mta_q, 'tradeAmt': tradeAmt, 'MTA_mean': MTA_mean, 'STA_mean': STA_mean, 'ret_30m': ret_30m, 'r2': R2, 'MTA_coef': MTA_coef, 'STA_coef': STA_coef})
    return(df)
...
if __name__ == '__main__':
    ...
    #print(summary(df))
    summary_mta(df)
    #summary_mta(df).to_csv('/data/home/jianw/OrderAnalysis/20220117_30min/decile_data/constant_mta_summary_orderside%s.%s.%s.csv' %(order_side, sdate, edate), index = False)
