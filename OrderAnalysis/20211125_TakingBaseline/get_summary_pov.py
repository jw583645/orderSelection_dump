...
def order_selection(df_side, method,demean, order_amt_all, order_side, top, sta_wt, date):
    ...
    #df_side = df_side.sort_values(by = ['interval', 'skey','mta', 'time'], ascending=[True, True, False, True])
    df_side = df_side.sort_values(by = ['interval', 'skey','alpha_l1', 'sta', 'time'], ascending=[True, True, False, False, True])
    ...
    if method == 'allDay':
        ...
        df_side = df_side.sort_values(by = ['interval','alpha_l1'], ascending=[True, False])
...
def saveSummary(sdate, edate, inSample, top, order_cap, order_side, method, ret, sta_wt):
    ...
    df.to_csv('/data/home/jianw/OrderAnalysis/20211125_TakingBaseline/stats_summary/baseline_POV/sta_wts/%s.%s.%s.staWt_%s.%s.%s.%s.csv'%(method,ret, order_side, sta_wt, sdate, edate, inSample_dic[inSample] ), index = False)
