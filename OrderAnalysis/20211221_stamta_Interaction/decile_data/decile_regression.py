...
def summary_mta(decile):
    ...
    df_local = pd.read_parquet('/data/home/jianw/OrderAnalysis/20211221_stamta_Interaction/decile_data/orderside%s/decile%s.parquet'%(order_side, i))
    ...
    df = pd.DataFrame({'mta_q':mta_q, 'tradeAmt': tradeAmt, 'MTA_mean': MTA_mean, 'STA_mean': STA_mean, 'Spread_mean': Spread_mean, 'ret_1d': ret_1d, 'r2': R2, 'MTA_coef': MTA_coef, 'STA_coef': STA_coef, 'Spread_coef': Spread_coef})
...
if __name__ == '__main__':
    ...
    df.to_csv('/data/home/jianw/OrderAnalysis/20211221_stamta_Interaction/decile_data/wls_constant_spread_mta_summary_orderside%s.%s.%s.csv' %(order_side, sdate, edate), index = False)
