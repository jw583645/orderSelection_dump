...
def order_selection(df_side, method,demean, order_amt_all, order_side, top, sta_wt, date):
    ...
    df_side['alpha_l1'] = np.nan
    df_side['alpha_l1'] = np.where(df_side['order_side'] == 1, (1-sta_wt) * df_side['alp_1d'] +  sta_wt*df_side['prev_yHatBuy90'], df_side['alpha_l1'])
    df_side['alpha_l1'] = np.where(df_side['order_side'] == 2, -(1-sta_wt) * df_side['alp_1d'] +  sta_wt*df_side['prev_yHatSell90'], df_side['alpha_l1'])
    
    #print(df_side[['skey','order_side','alpha', 'alpha_l1', 'alp_1d', 'prev_yHatBuy90', 'prev_yHatSell90']])
    
    df_side['mta'] = np.where(order_side ==1, df_side['alp_1d'], -df_side['alp_1d'])
    
    #df_side = df_side.sort_values(by = ['interval', 'skey','alpha_l1', 'time'], ascending=[True, True, False, True])
    df_side = df_side.sort_values(by = ['interval', 'skey','sta', 'time'], ascending=[True, True, False, True])
    #df_side = df_side.sort_values(by = ['interval', 'skey','alpha_l1', '90s_ret', 'time'], ascending=[True, True, False, False, True])
    #df_side = df_side.sort_values(by = ['interval', 'skey','alpha_l1', '90s_ret', 'time'], ascending=[True, True, False, False, True])
    ...
    if method == 'allDay':
        df_side = df_side.sort_values(by = ['alpha_l1'], ascending = False)
        ...
        #df_side = df_side.sort_values(by = ['interval','alpha_l1', '90s_ret'], ascending=[True, False, False])
        ...
        #df_side = df_side.sort_values(by = ['interval','mta', 'time'], ascending=[True, False, True])
        ...
        df_side = df_side.sort_values(by = ['skey','alpha_l1'], ascending=[True, False])
    ...
    #df_side['threshold'] = df_side['order_amt_all'] * top/0.2
    #print(df_side)
    
    df_side_top = df_side.loc[df_side['order_amt_cumsum']<=df_side['selection_amt']]
    
    #print('!!!!!!!!!!!!!!!!!!!!!!!!!')
    #os.mkdir('/data/home/jianw/OrderAnalysis/20211125_TakingBaseline/stats_summary/baseline_POV/sta_wts/top_orders/staWt_%s/%s.%s.%s'%(sta_wt, method, demean, order_side))
    ...
    df_side_top.to_parquet('/data/home/jianw/OrderAnalysis/20211125_TakingBaseline/stats_summary/baseline_POV/sta_wts/top_orders/staWt_%s_%s.%s.%s/top_orders.%s.parquet'%(sta_wt, method, demean, order_side, date))   
    return(df_side_top)
    
def gen_summary(tuple):
    date, order_side, top, order_cap, method, demean, sta_wt  = tuple
    print(date)
    try:
    ...
    ## Select Top Orders 
    df_side_top = order_selection(df_side, method, demean, order_amt_all, order_side, top, sta_wt,  date)
    #print('!!!!!!!!!!!!!!!!!!!!!!!!!!')
    #print(df_side_top)
...
def saveSummary(sdate, edate, inSample, top, order_cap, order_side, method, ret, sta_wt):
    ...
    tuples = []
    for date in dates:
        tuples.append((date, order_side, top, order_cap, method, ret, sta_wt))
    print(tuples)
    ...
    df.to_csv('/data/home/jianw/OrderAnalysis/20211125_TakingBaseline/stats_summary/baseline_POV/sta_wts/predictSTA.%s.%s.%s.staWt_%s.%s.%s.%s.csv'%(method,ret, order_side, sta_wt, sdate, edate, inSample_dic[inSample] ), index = False)
...
if __name__ == '__main__':
    ...
    #rets = ['raw','minute']
    rets = ['minute']
    #sta_wts = [0, 0.25,  1, 4, 100]
    #sta_wts = [0.2, 0.33, 0.67,  1.5, 3, 5]
    #sta_wts = [0, 0.33, 0.5, 0.67, 1]
    #sta_wts = [0, 0.33,0.67, 1]
    #sta_wts = [0, 0.1, 0.2, 0.33, 0.4, 0.5, 0.6, 0.67, 0.8, 0.9, 1]
    #sta_wts = [0, 0.33,0.67]
    sta_wts = [0]
    inSample = 1
        ...
        for method in methods:
            for ret in rets:
                for sta_wt in sta_wts:
                    saveSummary(sdate, edate, inSample, top, order_cap, order_side, method, ret, sta_wt)
