...
def gen_summary(tuple):
    univ, date, order_side, top, order_cap, method, demean, wt_mta30m, wt_sta  = tuple
    print(date)
    try:
        df = pd.read_parquet('/data/home/jianw/monetization_research/rawData_jian/concat_data/%s/%s.parquet' %(univ,date.replace('-', '')))
...
def saveSummary(univ, sdate, edate, inSample, top, order_cap, order_side, method, ret, wt_mta30m, wt_sta):
    if os.path.exists("/data/home/jianw/OrderAnalysis/20220221_newBaseline/stats_summary/%s/top.%s.%s.orderside%s.%s-%s.2021-01-01.2021-10-29.IS.csv" %(univ, method, ret, order_side, wt_mta30m, wt_sta)):
    ...
    tuples = []
    for date in dates:
        tuples.append((univ, date, order_side, top, order_cap, method, ret, wt_mta30m, wt_sta))
    print(tuples)
    ...
    df.to_csv('/data/home/jianw/OrderAnalysis/20220221_newBaseline/stats_summary/%s/top.%s.%s.orderside%s.%s-%s.%s.%s.%s.csv'%(univ,method,ret, order_side, wt_mta30m, wt_sta, sdate, edate, inSample_dic[inSample] ), index = False)
            ...
            for ret in rets:
                for mta_wt_30m in mta_wts_30m:
                    for sta_wt in sta_wts:
                        saveSummary(univ,sdate, edate, inSample, top, order_cap, order_side, method, ret, mta_wt_30m, sta_wt)
