...
def calPortion(date):
    ...
    try:
        os.makedirs("/data/home/jianw/OrderAnalysis/20220303_mtaRealization/data/daily/%s" %(idx))
    except:
        pass
    df.to_parquet("/data/home/jianw/OrderAnalysis/20220303_mtaRealization/data/daily/%s/%s.parquet" %(idx, date))
    ...
    '''
def getSummary(date):
    df = pd.read_parquet("/data/home/jianw/OrderAnalysis/20220303_mtaRealization/data/daily/%s/%s.parquet" %(idx, date))
    
    df['topBar'] = df.groupby('interval')['alp_1d'].transform(lambda x: np.quantile(x, 1-top))
    df_top = df.loc[df['alp_1d']>=df['topBar']]

    
    df_top['retTop'] = df_top.groupby('interval')['ret_50s'].transform(lambda x: np.quantile(x, 0.8))
    df_top['retBottom'] = df_top.groupby('interval')['ret_50s'].transform(lambda x: np.quantile(x, 0.2))
    
    #df_top = df_top.loc[(df_top['ret_50s'] >= df_top['retBottom']) & (df_top['ret_50s'] <= df_top['retTop'])]
    df_top = df_top.loc[(df_top['ret_50s'] < df_top['retBottom'])]
    #df_top = df_top.loc[(df_top['ret_50s'] > df_top['retTop'])]
    ...
    mean_50s = df_top['ret_50s'].mean() * 10000
    mean_50s_1d = df_top['ret_50s_1d'].mean() * 10000
    mean_trade_1d2 = df_top['ret_trade_1d2'].mean() * 10000
    mean_sta = df_top['yHatBuy90'].mean() * 10000
    
    df = pd.DataFrame({'date': date, 'mean_30s': mean_30s, 'mean_50s': mean_50s,  'mean_1d': mean_1d, 'mean_30s_1d': mean_30s_1d, 'mean_50s_1d': mean_50s_1d, 'mean_trade_1d2': mean_trade_1d2, 
        'mean_sta' : mean_sta}, index = [0])
...
if __name__ == '__main__':
    ...
    #calPortion('2021-06-22')
    
    '''
    P = mp.Pool(96)
    #P.map(loadAlphaF90, list(set(daily_data['date'])))
    P.map(calPortion, dates)
    P.close()
    P.join()
    '''
