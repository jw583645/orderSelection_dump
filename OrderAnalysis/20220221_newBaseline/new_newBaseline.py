...
def order_selection(df_side, method,demean, order_amt_all, order_side, top):
    ...
    df_side['pov'] = np.where(df_side['interval']<= 94500000000, 0.4, 0.2)
...
def saveSummary(univ, sdate, edate, inSample, top, order_cap, order_side, method, ret, wt_mta30m, wt_sta):
    ...
    df.to_csv('/data/home/jianw/OrderAnalysis/20220221_newBaseline/stats_summary/%s/top.%s.%s.orderside%s.%s-%s.%s.%s.%s_new.csv'%(univ,method,ret, order_side, wt_mta30m, wt_sta, sdate, edate, inSample_dic[inSample] ), index = False)
