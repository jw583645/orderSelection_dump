...
for j in range(10):
    ...
    print('decile %s'%(j+1))
    df = pd.read_parquet("/data/home/jianw/OrderAnalysis/20220117_30min//decile_data/orderside1/decile%s.parquet" %(j+1))
    '''
    ...
    df = pd.DataFrame({'mta_q': j+1, 'price_level': price_level[0:6], 'trade_wts': trade_wts, 'rets (bps)' : rets})
    df_l.append(df)
    print(df)
