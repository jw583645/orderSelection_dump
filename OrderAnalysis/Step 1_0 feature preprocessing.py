...
def collapse_sta(skey, date):
    ...
    df.sort_values('ordering', inplace = True)
    ...
    ## features for jumps in sta and raw price
    df['prev_safe_bid1p'] = np.where(df['prev_bid1p'] == 0, df['prev_ask1p'], df['prev_bid1p'])
    df['prev_safe_ask1p'] = np.where(df['prev_ask1p'] == 0, df['prev_bid1p'], df['prev_ask1p'])
    df['prev_sta_mid'] = (df['prev_safe_bid1p'] + df['prev_safe_ask1p']) / 2
    df['prev_mid_up_cur'] = (df['prev_sta_mid'] > df['prev_sta_mid'].shift(1)).astype(int)
    df['prev_mid_down_cur'] = (df['prev_sta_mid'] < df['prev_sta_mid'].shift(1)).astype(int)
    df['prev_mid_up_10_orders'] = df['prev_mid_up_cur'].rolling(10, min_periods = 1).sum()
    df['prev_mid_down_10_orders'] = df['prev_mid_down_cur'].rolling(10, min_periods = 1).sum()

    df['sta_jump_up_cur'] = (df['prev_yHatBuy90'] - df['prev_yHatBuy90'].shift(1) > 1e-3).astype(int)
    df['sta_jump_down_cur'] = (df['prev_yHatBuy90'] - df['prev_yHatBuy90'].shift(1) < -1e-3).astype(int)
    df['sta_jump_up_10_orders'] = df['sta_jump_up_cur'].rolling(10, min_periods = 1).sum()
    df['sta_jump_down_10_orders'] = df['sta_jump_down_cur'].rolling(10, min_periods = 1).sum()

    df['datetime'] = pd.to_datetime(df['date'].astype(str) + df['prev_time'].astype(int).astype(str), format = '%Y%m%d%H%M%S%f')
    df.set_index(pd.DatetimeIndex(df['datetime']), inplace = True)
    df['prev_mid_up_5min'] = df['prev_mid_up_cur'].rolling(window = '300000ms', closed = 'both').sum()
    df['prev_mid_down_5min'] = df['prev_mid_down_cur'].rolling(window = '300000ms', closed = 'both').sum()

    df['sta_jump_up_5min'] = df['sta_jump_up_cur'].rolling(window = '300000ms', closed = 'both').sum()
    df['sta_jump_down_5min'] = df['sta_jump_down_cur'].rolling(window = '300000ms', closed = 'both').sum()

    df.reset_index(drop = True, inplace = True)
    #df.to_csv('/data/home/jianw/project_mkt_impact/output/test.csv', index = False)
    return df
...
def merge_alpha(pair):
    ...
    ## mta jump features
    mta['1d_jump_up_cur'] = (mta['alp_1d'] - mta['alp_1d'].shift(1) > 1e-3).astype(int)
    mta['1d_jump_down_cur'] = (mta['alp_1d'] - mta['alp_1d'].shift(1) < -1e-3).astype(int)
    mta['1d_jump_up_5min'] = mta['1d_jump_up_cur'].rolling(5, min_periods = 1).sum()
    mta['1d_jump_down_5min'] = mta['1d_jump_down_cur'].rolling(5, min_periods = 1).sum()
    #print(mta['1d_jump_up_cur'])
    
    mta.rename(columns = {'mid': 'mta_mid_price', 'bid1p': 'mta_bid1p', 'ask1p': 'mta_ask1p'}, inplace = True)
    df = pd.concat([sta, mta[['skey', 'prev_time', 'min', 'alp_1d', 'alp_2d', 'beta', 'idx_openv_ND1', 'stk_openv_ND1', 'idx_N30min', 'stk_N30min', 'stk_dclose_ND1', 'idx_dclose_ND1', 'idx_openv_ND2', 'stk_openv_ND2', 
    'mta_mid_price', 'mta_bid1p', 'mta_ask1p', 'label', '1d_jump_up_cur', '1d_jump_down_cur', '1d_jump_up_5min', '1d_jump_down_5min']]])
    ...
    '1d_jump_up_cur', '1d_jump_down_cur', '1d_jump_up_5min', '1d_jump_down_5min']:
    ...
    'idx_openv_ND1', 'stk_openv_ND1', 'idx_N30min', 'stk_N30min', 'stk_dclose_ND1', 'idx_dclose_ND1', 'idx_openv_ND2', 'stk_openv_ND2', 'mta_mid_price', 'mta_bid1p', 'mta_ask1p',
    'prev_mid_up_cur', 'prev_mid_down_cur', 'prev_mid_up_10_orders', 'prev_mid_down_10_orders', 'prev_mid_up_5min', 'prev_mid_down_5min', 
    'sta_jump_up_cur', 'sta_jump_down_cur', 'sta_jump_up_10_orders', 'sta_jump_down_10_orders', 'sta_jump_up_5min', 'sta_jump_down_5min',
    '1d_jump_up_cur', '1d_jump_down_cur', '1d_jump_up_5min', '1d_jump_down_5min']].isnull().any().any()
    except:
        ...
        'prev_mid_up_cur', 'prev_mid_down_cur', 'prev_mid_up_10_orders', 'prev_mid_down_10_orders', 'prev_mid_up_5min', 'prev_mid_down_5min', 
    'sta_jump_up_cur', 'sta_jump_down_cur', 'sta_jump_up_10_orders', 'sta_jump_down_10_orders', 'sta_jump_up_5min', 'sta_jump_down_5min',
    '1d_jump_up_cur', '1d_jump_down_cur', '1d_jump_up_5min', '1d_jump_down_5min']].isnull().any())
    ...
    try:
        os.makedirs('/data/home/jianw/OrderAnalysis/data_feature/{}'.format(date))
    except:
        pass

    df.to_csv('/data/home/jianw/OrderAnalysis/data_feature/{}/{}.csv.gz'.format(date, tstock), index = False)
...
if __name__ == '__main__':
    ...
    #merge_alpha((2000826, 20210104))
    #collapse_sta(2000826, 20210105)
