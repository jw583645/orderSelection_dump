...
def getSummary(date):
    ...
    ## time of the day
    df = df.loc[(df['time'] >= 130000000000) & (df['time'] <= 143000000000)]
...
if __name__ == '__main__':
    ...
    for idx in Idxs:
        ...
        df = pd.concat(df_l)
        df.to_csv('/data/home/jianw/OrderAnalysis/20220307_orderLevelPriceRealization/data/summaryData/%s_summary.afternoon.%s.%s.csv' %(idx, sdate, edate), index = False)
    '''
