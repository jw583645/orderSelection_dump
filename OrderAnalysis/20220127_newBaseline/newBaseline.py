...
def order_selection(df_side, method,demean, order_amt_all, order_side, top):
    ...
    df_side = df_side.sort_values(by = ['interval', 'skey','alpha', 'time'], ascending=[True, True, True, True])    
    ...
    if method == 'allDay':
        ...
        df_side = df_side.sort_values(by = ['interval','alpha'], ascending=[True, True])
...
def saveSummary(sdate, edate, inSample, top, order_cap, order_side, method, ret, wt_mta30m, wt_sta):
    if os.path.exists("/data/home/jianw/OrderAnalysis/20220127_newBaseline/stats_summary/mtasta_new/bottome.%s.%s.orderside%s.%s-%s.2021-01-01.2021-10-29.IS.csv" %(method, ret, order_side, wt_mta30m, wt_sta)):
    ...
    df.to_csv('/data/home/jianw/OrderAnalysis/20220127_newBaseline/stats_summary/mtasta_new/bottome.%s.%s.orderside%s.%s-%s.%s.%s.%s.csv'%(method,ret, order_side, wt_mta30m, wt_sta, sdate, edate, inSample_dic[inSample] ), index = False)
